function generateSearch(search, button) {
    let result = `
        <header>
            <el-form :model="searchFormData" :inline="true" label-position="${search.labelPosition ?? 'left'}" label-width="${search.labelWidth ?? '80px'}" ref="searchForm">
...
function generateOperationList(operationList) {
    return operationList.map((operation) => {
        ...
        return `<router-link type="primary" :to="{path: '${operation.path}'${operation.query ? `, query: {${generateQuery(operation.query)}}` : ''}}">${operation.text}</router-link>`
    }).join('\n')
}
